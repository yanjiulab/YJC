...
int main() {
    ...
    local.nl_groups = RTMGRP_LINK | RTMGRP_IPV4_IFADDR |
                      RTMGRP_IPV4_ROUTE;  // set groups we interested in
    ...
    while (1) {
        ...
        if (msg.msg_namelen !=
            sizeof(local)) {  // check message length, just in case
        ...
        for (h = (struct nlmsghdr *)buf;
            ...
            if ((h->nlmsg_type == RTM_NEWROUTE) ||
                (h->nlmsg_type ==
                 RTM_DELROUTE)) {  // some changes in routing table
                ...
                if (tb[IFLA_IFNAME]) {  // validation
                    ifName = (char *)RTA_DATA(
                        tb[IFLA_IFNAME]);  // get network interface name
                }

                if (ifi->ifi_flags &
                    IFF_UP) {  // get UP flag of the network interface
                    ifUpp = (char *)"UP";
                } else {
                    ifUpp = (char *)"DOWN";
                }
            ...
            h = (struct nlmsghdr *)((char *)h +
                                    NLMSG_ALIGN(len));  // get next message
        }
