...
static inline bool is_default_prefix4(const struct prefix_ipv4* p) {
    return p && p->family == AF_INET && p->prefixlen == 0 && p->prefix.s_addr == INADDR_ANY;
}

static inline bool is_default_prefix6(const struct prefix_ipv6* p) {
    return p && p->family == AF_INET6 && p->prefixlen == 0 && memcmp(&p->prefix, &in6addr_any, sizeof(struct in6_addr)) == 0;
}
