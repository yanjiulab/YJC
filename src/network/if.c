...
int if_is_operative(const struct interface* ifp) {
    return ((ifp->flags & IFF_UP) && (((ifp->flags & IFF_RUNNING) && (ifp->ptm_status || !ifp->ptm_enable)) || !CHECK_FLAG(ifp->status,
...
int if_is_no_ptm_operative(const struct interface* ifp) {
    return ((ifp->flags & IFF_UP) && ((ifp->flags & IFF_RUNNING) || !CHECK_FLAG(ifp->status,
                                                                                ZEBRA_INTERFACE_LINKDETECTION)));
}
...
#define IFF_OUT_LOG(X, STR)                       \
    if (flag & (X)) {                             \
        if (separator)                            \
            strncat(logbuf, ",", sizeof(logbuf)); \
        else                                      \
            separator = 1;                        \
        strncat(logbuf, STR, sizeof(logbuf));     \
    }
...
struct nbr_connected* nbr_connected_new(void) {
    return calloc(1, sizeof(struct nbr_connected));
}
...
struct connected* connected_lookup_prefix(struct interface* ifp,
    ...
    for (ALL_LIST_ELEMENTS_RO(ifp->connected, cnode, c)) {
        if (c->address && (c->address->family == addr->family) && prefix_match(CONNECTED_PREFIX(c), addr) && (!match || (c->address->prefixlen > match->address->prefixlen)))
            match = c;
    }
    return match;
}
...
struct connected* connected_get_linklocal(struct interface* ifp) {
    ...
    for (ALL_LIST_ELEMENTS_RO(ifp->connected, n, c)) {
        if (c->address->family == AF_INET6 && IN6_IS_ADDR_LINKLOCAL(&c->address->u.prefix6))
            break;
    }
    return c;
}
...
struct if_link_params* if_link_params_enable(struct interface* ifp) {
    ...
    iflp->default_bw =
        ((ifp->bandwidth ? ifp->bandwidth : DEFAULT_BANDWIDTH) * TE_MEGA_BIT / TE_BYTE);
...
struct if_link_params* if_link_params_init(struct interface* ifp) {
    ...
    iflp = calloc(1, sizeof(struct if_link_params));
