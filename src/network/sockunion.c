...
union sockunion* sockunion_str2su(const char* str) {
    union sockunion* su = calloc(1, sizeof(union sockunion));
...
int sockunion_stream_socket(union sockunion* su) {
    ...
    if (sock < 0)
        log_error("can't make socket sockunion_stream_socket");
...
int sockopt_reuseport(int sock) {
    ...
    ret = setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, (void*)&on,
                     sizeof(on));
    if (ret < 0) {
        log_error("can't set sockopt SO_REUSEPORT to socket %d", sock);
        return -1;
    }
    return 0;
}
...
int sockopt_ttl(int family, int sock, int ttl) {
    ...
    if (family == AF_INET) {
        ret = setsockopt(sock, IPPROTO_IP, IP_TTL, (void*)&ttl,
                         sizeof(int));
        if (ret < 0) {
            log_error("can't set sockopt IP_TTL %d to socket %d",
                      ttl, sock);
            return -1;
        }
        return 0;
    ...
    if (family == AF_INET6) {
        ret = setsockopt(sock, IPPROTO_IPV6, IPV6_UNICAST_HOPS,
                         (void*)&ttl, sizeof(int));
        if (ret < 0) {
            log_error("can't set sockopt IPV6_UNICAST_HOPS %d to socket %d",
                      ttl, sock);
            return -1;
        }
        return 0;
...
int sockopt_v6only(int family, int sock) {
    ...
    if (family == AF_INET6) {
        ret = setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (void*)&on,
                         sizeof(int));
        if (ret < 0) {
            log_error("can't set sockopt IPV6_V6ONLY to socket %d",
                      sock);
            return -1;
        }
        return 0;
...
union sockunion* sockunion_getsockname(int fd) {
    ...
    log_error("Unexpected AFI received(%d) for sockunion_getsockname call for fd: %d",
...
union sockunion* sockunion_getpeername(int fd) {
    ...
    log_error("Unexpected AFI received(%d) for sockunion_getpeername call for fd: %d",
              name.sa.sa_family, fd);
    return NULL;
}
