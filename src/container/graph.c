...
graph_edge_t* graph_add_edge(graph_t* graph, graph_node_t* from, graph_node_t* to, void* attr, void (*del)(void*)) {
    ...
    for (unsigned int i = vector_active(graph->nodes); i--; /**/) {
        if (from == vector_slot(graph->nodes, i))
            f = true;
        if (to == vector_slot(graph->nodes, i))
            t = true;
    }
    if (!f || !t)
        return e;
    ...
    case Graph:
        ...
        if (!graph_has_edge(graph, to, from))
...
graph_t* graph_reverse(graph_t* graph, bool copy) {
    if (graph->type == Graph || graph->type == MultiGraph)
        return graph;
...
graph_t* graph_gen_grid(int m, int n, graph_type_t t) {
    if (m == 1)
        return graph_gen_cycle(n, t);
    if (n == 1)
        return graph_gen_cycle(m, t);
    int i, j, k, u, d, l, r;
    ...
    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++) {
            ...
            if (j == n - 1)
                r = -1;
            if (u != -1)
                graph_add_edge(g, vector_slot(g->nodes, k), vector_slot(g->nodes, u), 0, NULL);
            if (r != -1)
                graph_add_edge(g, vector_slot(g->nodes, k), vector_slot(g->nodes, r), 0, NULL);
            if (d != -1)
                graph_add_edge(g, vector_slot(g->nodes, k), vector_slot(g->nodes, d), 0, NULL);
            if (l != -1)
                graph_add_edge(g, vector_slot(g->nodes, k), vector_slot(g->nodes, l), 0, NULL);
        }
    }
    return g;
}
