...
static struct listnode *listnode_new(void) { return calloc(1, sizeof(struct listnode)); }

/* Free listnode. */
static void listnode_free(struct listnode *node) { free(node); }
...
struct listnode *listnode_lookup(struct list *list, void *data) {
    ...
    for (node = listhead(list); node; node = listnextnode(node))
        if (data == listgetdata(node)) return node;
    return NULL;
}

void list_dump(struct list *list, const char *format) {
    struct listnode *node, *nnode;
    void *data;
    printf("[");
    list_foreach(list, node, nnode, data) {
