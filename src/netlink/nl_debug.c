...
static void dump_on_off(uint32_t ival, const char* prefix) {
    log_debug("%s%s", prefix, (ival != 0) ? "on" : "off");
}
...
next_rta:
    ...
    log_debug("      linkinfo [len=%d (payload=%zu) type=(%d) %s]",
...
next_rta:
    ...
    log_debug("    rta [len=%d (payload=%zu) type=(%d) %s]", rta->rta_len,
    ...
    case IFLA_ADDRESS:
        ...
        for (it = 0; it < plen; it++) {
            ...
            strncat(dbuf, bytestr, sizeof(dbuf));
            datap++;
        }
        /* Remove trailing ':'. */
        if (dbuf[0])
...
next_rta:
    ...
    case RTA_MFC_STATS:
        ...
        log_debug("      pkts=%ju bytes=%ju wrong_if=%ju",
...
next_rta:
    ...
    case IFA_LABEL:
        log_debug("      %s", (const char*)RTA_DATA(rta));
        break;
...
// 	log_debug("  vni_start %u, vni_end %u", vni_start, vni_end);
...
next_rta:
    ...
    case NHA_GROUP:
        ...
        if (count == 0 || (count * sizeof(*nhgrp)) != RTA_PAYLOAD(rta)) {
            log_debug("      invalid nexthop group received");
            return;
        }

        for (i = 0; i < count; i++)
            log_debug("      id %d weight %d", nhgrp[i].id,
                      nhgrp[i].weight);
        break;
    case NHA_ENCAP_TYPE:
        u16v = *(uint16_t*)RTA_DATA(rta);
        log_debug("      %s", lwt_type2str(u16v));
        break;
    case NHA_GROUP_TYPE:
        u16v = *(uint16_t*)RTA_DATA(rta);
        log_debug("      %s", nhg_type2str(u16v));
        break;
    case NHA_BLACKHOLE:
        /* NOTHING */
        break;
    ...
    case NHA_GATEWAY:
        ...
        default:
            log_debug("      invalid family %d", nhm->nh_family);
            break;
        }
        break;
    case NHA_ENCAP:
        /* TODO: handle MPLS labels. */
        log_debug("      unparsed MPLS labels");
        break;
    case NHA_GROUPS:
        /* TODO: handle this message. */
        log_debug("      unparsed GROUPS message");
        break;
...
next_rta:
    ...
    case FRA_UID_RANGE:
        ...
        if (u_range->start == u_range->end)
            log_debug("      %u", u_range->start);
        else
            log_debug("      %u-%u", u_range->start, u_range->end);
        break;
    ...
    case FRA_DPORT_RANGE:
        ...
        if (p_range->start == p_range->end)
            log_debug("      %u", p_range->start);
        else
            log_debug("      %u-%u", p_range->start, p_range->end);
        break;
