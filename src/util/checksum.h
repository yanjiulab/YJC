...
static inline uint16_t in_cksum_with_ph4(const struct ipv4_ph *ph, const void *data, size_t nbytes) {
    struct iovec iov[2];
...
static inline uint16_t in_cksum_with_ph6(const struct ipv6_ph *ph, const void *data, size_t nbytes) {
    struct iovec iov[2];
...
extern uint16_t fletcher_checksum(uint8_t *, const size_t len, const uint16_t offset);
