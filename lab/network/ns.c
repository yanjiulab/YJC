...
static struct ns* ns_get_created_internal(struct ns* ns, char* name,
    ...
    if (ns_debug) {
        if (ns->ns_id != NS_UNKNOWN)
            log_info("NS %u is created.", ns->ns_id);
        else
            log_info("NS %s is created.", ns->name);
    }
...
static int ns_enable_internal(struct ns* ns, void (*func)(ns_id_t, void*)) {
    if (!ns_is_enabled(ns)) {
        ...
        if (!ns_is_enabled(ns)) {
            log_error("Can not enable NS %u: %s!", ns->ns_id, strerror(errno));
            return 0;
        }

        /* Non default NS. leave */
        if (ns->ns_id == NS_UNKNOWN) {
            log_error("Can not enable NS %s %u: Invalid NSID",
                      ns->name, ns->ns_id);
            return 0;
        }
        if (func)
        ...
        if (ns_debug) {
            if (have_netns())
                log_info("NS %u is associated with NETNS %s.",
                         ns->ns_id, ns->name);
            log_info("NS %u is enabled.", ns->ns_id);
        }
...
static void ns_disable_internal(struct ns* ns) {
    if (ns_is_enabled(ns)) {
        if (ns_debug)
            log_info("NS %u is to be disabled.", ns->ns_id);
...
ns_id_t ns_map_nsid_with_external(ns_id_t ns_id, bool map) {
    ...
    ns_map = calloc(1, sizeof(struct ns_map_nsid));
    /* increase vrf_id
...
void ns_delete(struct ns* ns) {
    if (ns_debug)
        log_info("NS %u is to be deleted.", ns->ns_id);
...
char* ns_netns_pathname(const char* name) {
    ...
    if (!result) {
        log_warn("Invalid pathname for %s: %s", pathname, strerror(errno));
        return NULL;
    }
    ...
    if (check_base != NULL && strlen(check_base) + 1 > NS_NAMSIZ) {
        ...
        log_warn("NS name (%s) invalid: too long (>%d)", check_base, NS_NAMSIZ - 1);
        return NULL;
    }
...
void ns_init_management(ns_id_t default_ns_id, ns_id_t internal_ns) {
    ...
    if (!default_ns) {
        log_error("%s: failed to create the default NS!",
                  __func__);
        exit(1);
    }
    if (have_netns()) {
    ...
    default_ns->name = strdup(NS_DEFAULT_NAME);
    if (ns_debug)
        log_info("%s: default NSID is %u", __func__,
                 default_ns->ns_id);

    /* Enable the default NS. */
    if (!ns_enable(default_ns, NULL)) {
        log_error("%s: failed to enable the default NS!",
                  __func__);
        exit(1);
    }
}
