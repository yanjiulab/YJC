...
struct vrf* vrf_get(vrf_id_t vrf_id, const char* name) {
    ...
    if (vrf && vrf_id != VRF_UNKNOWN && vrf->vrf_id != VRF_UNKNOWN && vrf->vrf_id != vrf_id) {
        log_debug("VRF_GET: avoid %s creation(%u), same name exists (%u)",
    ...
    if (vrf == NULL) {
        vrf = calloc(1, sizeof(struct vrf));
...
struct vrf* vrf_update(vrf_id_t new_vrf_id, const char* name) {
    ...
    if (name)
        vrf = vrf_lookup_by_name(name);
    if (vrf && new_vrf_id != VRF_UNKNOWN && vrf->vrf_id != VRF_UNKNOWN && vrf->vrf_id != new_vrf_id) {
        if (debug_vrf) {
            log_debug(
...
void vrf_delete(struct vrf* vrf) {
    if (debug_vrf)
        log_debug("VRF %s(%u) is to be deleted.", vrf->name,
                  vrf->vrf_id);
...
int vrf_enable(struct vrf* vrf) {
    ...
    if (debug_vrf)
        log_debug("VRF %s(%u) is enabled.", vrf->name, vrf->vrf_id);
...
void vrf_disable(struct vrf* vrf) {
    ...
    if (debug_vrf)
        log_debug("VRF %s(%u) is to be disabled.", vrf->name,
                  vrf->vrf_id);
...
static void vrf_autocomplete(vector comps, struct cmd_token* token) {
    ...
    vector_set(comps, strdup(vrf->name));
}
...
void vrf_init(int (*create)(struct vrf*), int (*enable)(struct vrf*),
    ...
    ns_init();
    if (debug_vrf)
        log_debug("%s: Initializing VRF subsystem", __func__);
    ...
    if (!default_vrf) {
        log_error("vrf_init: failed to create the default VRF!");
        exit(1);
    }
    ...
    if (!vrf_enable(default_vrf)) {
        log_error("vrf_init: failed to enable the default VRF!");
        exit(1);
    }
}
...
void vrf_terminate(void) {
    ...
    if (debug_vrf)
        log_debug("%s: Shutting down vrf subsystem", __func__);
...
int vrf_socket(int domain, int type, int protocol, vrf_id_t vrf_id,
    ...
    if (ret < 0)
        log_error("%s: Can't switch to VRF %u (%s)",
                  __func__, vrf_id, strerror(errno));

    ret = socket(domain, type, protocol);
    save_errno = errno;
    ret2 = vrf_switchback_to_initial();
    if (ret2 < 0)
        log_error("%s: Can't switchback from VRF %u (%s)", __func__,
                  vrf_id, strerror(errno));
    errno = save_errno;
    if (ret <= 0)
        return ret;
...
int vrf_sockunion_socket(const union sockunion* su, vrf_id_t vrf_id, const char* interfacename) {
    int ret, save_errno, ret2;
